...
- I²C：V3 使用两条 MCU I²C 控制器并分别用 `embassy_sync::Mutex` + `static_cell::StaticCell` 构建共享访问。
  - `SDA1/SCL1 = GPIO8/GPIO9`：输入 INA226、前面板、OUT3/OUT4 输出模块传感器；
  - `SDA0/SCL0 = GPIO14/GPIO13`：主板 `TCA6408A@0x20` hub-sideband、OUT1/OUT2 输出模块传感器。
- LCD：显示初始化成功后，先进入 boot self-check 页，不再把 dashboard 首帧当作“系统一定正常”的信号。
  - 当前 V3 主板由 MCU 直接驱动 `DC/MOSI/SCLK/BLK`；
  - LCD `CS/RES` 由前面板 `TCA6408A@0x21` 控制：`P6=CS` 保持输出低，`P5=RES` 启动时低脉冲后释放为输出高；
  - `GPIO13/GPIO14` 是 `SCL0/SDA0`，不得作为 LCD `CS/RST` 使用；
  - GC9D01 驱动实例使用 no-op reset pin，避免把 `GPIO35/RESET#` 误当 LCD reset 再次拉低。
...
5) 仅在 `VIN ready` 时初始化主板 CH335F sideband 与四路端口扫描：
   ...
   - 若 `TCA6408A@0x20` 离线，runtime 进入 degraded manual mode：owner 显式开启的端口可保持输出，但 `OVCUR#` 注入不可用，控制面必须继续报告 sideband fault；
...
- 总线分配以主板网表为准：通道 1/2 位于 `SDA0/SCL0`，通道 3/4 位于 `SDA1/SCL1`。
- 逐通道初始化策略（串行）：
...
- MCU `GPIO21` 读取 ISOUSB211 `V1OK`，用于区分上游隔离侧电源状态：
  - `V1OK=low`：standalone/no-upstream，产品保持独立供电输出能力；
  - `V1OK=high`：upstream-managed，按 CH335F `PWREN#` 控制对应通道。
- 初始化默认释放所有 `OVCUR#`：
  - 输出寄存器 `0x01` 写 `0xFF`；
  - 极性寄存器 `0x02` 写 `0x00`；
...
- 运行期每路输出使能条件：
  - `VIN ready`；
  - 主板 `TCA6408A@0x20` 在线；若离线则进入 degraded manual mode，仅按 owner 手动状态与 OCP latch 放行输出；
  - 若 `V1OK=low`，进入 standalone 模式：四路输出不因 `PWREN#` 关闭；
  - 若 `V1OK=high`，进入 upstream-managed 模式：对应 `PWREN#` 为低的通道允许输出，为高的通道关闭输出；
  - 对应软件过流 latch 未置位。
- `OVCUR#` 控制策略：
...
- 主板 `RESET#` 使用 `GPIO35`，低电平复位主板侧相关器件和前面板 `TCA6408A@0x21`，启动时固件先输出低电平脉冲，再以推挽高电平保持释放，并记录释放电平。
- 前面板 `TCA6408A@0x21` 在线后，固件配置 `P5/P6` 为 LCD `RES/CS` 输出；按键输入使用 `P0..P4`。
- 主板风扇供电由 `TPS62933DRLR` buck 生成 `FAN_VCC`：
  - `GPIO2/FAN_EN` 直接驱动 buck `EN`，高电平使能；
  - `GPIO1/FAN_PWM` 通过 `R15 -> VCTRL/C16 -> R18` 注入 buck `FB`；
  - `FAN_PWM` 平均电压越高，`FB` 越高，`FAN_VCC` 越低，因此固件的速度百分比到硬件 duty 为反相映射；
  - `GPIO6/FAN_TOUCH` 通过 PCNT 计数，外部 4.7 kΩ 上拉到 3V3，并有 100 pF 到地滤波。
- 硬件快照将风扇运行期状态作为调试契约输出：内部温度、过温告警、`FAN_EN` 使能、tach 可信性、实测 RPM、目标 RPM、最大 RPM、控制速度百分比与反相后的硬件 PWM duty。
...
- V3 `BUZZER` 网络由 ESP32-S3 `GPIO7` 驱动；固件使用 LEDC 硬件 PWM 播放无源蜂鸣器音效。LEDC 外设由启动流程统一初始化，fan 使用 Timer0/Channel0，蜂鸣器使用 Timer1/Channel1，播放期间 CPU 只在音符边界调度，不生成半周期波形。
- GPIO7 空闲与每段音效播放完成后必须拉低，确保静音态稳定。
- 硬件快照输出蜂鸣器 driver ready、当前是否播放、one-shot tone、循环 alarm、频率与 duty，用于只读调试，不提供写寄存器或强制发声入口。
- 固件内置音效表来自 `tools/buzzer_audio_preview/` 试听页的推荐候选；运行时不读取 HTML、JSON、MIDI 或 WAV 试听产物。
- Boot 音：
  - boot self-check 结果非 `Fatal`，且阶段切换到 `Runtime` 后播放开机音；
  - `Fatal` 停留页不播放开机音。
- 前面板按键音：
  - Left/Right 选中通道移动成功时播放操作提示音；
  - Center 成功启用通道时播放通道上电音；
  - Center 成功禁用通道时播放通道断电音；
  - Center 尝试启用通道但因 OCP latch、全局 `PWR_SW_TARGET=Open`、CH335F sideband gate 或端口未就绪被拒绝时播放操作拒绝音，并记录拒绝原因；
  - Up/Down 当前无定义动作，保持静默。
- USB JSONL 与 network 控制面触发 `PortPowerSet(enabled=true)` 时播放通道上电音；触发 `PortPowerSet(enabled=false)` 或 `PortReplug` 时播放通道断电音。
- 告警音由运行期 reducer 选择唯一 active alarm，优先级为：
  1. `channel_short`；
  2. `over_temp`；
  3. `input_over_power`；
  4. `channel_over_5a`；
  5. one-shot 音效。
- 告警映射：
  - `channel_short` 复用现有低 VBUS OCP：`vbus < 3000mV && current > 100mA`；
  - `channel_over_5a` 在任一 active 通道的新鲜遥测 `current_ma >= 5000` 或 high-current OCP latch 时触发，按长间隔循环；
  - `over_temp` 复用 fan 安全阈值：80C 触发，75C 清除；
  - `input_over_power` 使用输入电源最新状态计算 `VIN * |I|`：100W 触发，90W 清除。
- 通道提示音使用 per-port hysteresis：
  - 插入：`vbus_mv >= 3300`；
  - 拔出：`vbus_mv < 3000`；
  - 3A 提示：首次 `current_ma >= 3000`，低于 `2800` 后复位；
  - 5A 提示：首次 `current_ma >= 5000`，低于 `4800` 后复位。
- 保护关断同一 tick 内必须抑制普通拔出/断电提示，只保留告警音。
- 可观测日志：
  - `buzzer.play: tone=<name>`；
  - `buzzer.alarm: active=<name|none>`；
  - `buzzer.alarm.play: tone=<name>`。

### 1.8.3 只读硬件底层快照

- 固件通过 Local USB JSONL `hardware.snapshot` 返回最新 `isolarail.hardware.snapshot.v1` 快照缓存；
- 快照是只读诊断接口，只汇总已有启动自检、运行期采样、I²C 探测和寄存器只读结果；
- 节点状态统一使用 `online` / `offline` / `skipped` / `error`，前面板 `TCA6408A@0x21` 与四路输出模块传感器允许离线；
- 单个节点离线只进入该节点状态与 reason，不得导致整包输出失败；
- Web 高级调试页位于 `/devices/:deviceId/debug/hardware`，作为设备路径下的 Advanced/Debug 入口，不替代主 dashboard/settings workflow；
- 详细 schema 与验收标准见 `docs/specs/n8x4q-hardware-diag-snapshot/SPEC.md`。
...
- 越界保护：启动资格及每个 `100 ms` 运行期保护采样中，若 `INA226.VBUS > 28.0 V`，视为输入过压，在下一保护采样周期内保持输入开关关闭并报告 `VIN OVP`；运行期过压会锁存输入关闭，直至重启或显式恢复策略。
...
- 当前 V3 板启动资格电流阈值：`I_IDLE_MAX_A = 30 mA`；
  ...
  - 旧的 `10 mA` 假设会把当前板误判成 `VIN OFF/FATAL`，导致 runtime 和 USB JSONL 永远不启动；
